...
// === [수정] 반환 타입 튜플에 int를 하나 더 추가하여 rasterize_points.h와 일치시킴 ===
...
RasterizeGaussiansVanillaCUDA(
	...
	const torch::Tensor& sh, const int degree, const int degree_t, 
	const torch::Tensor& campos, const float timestamp, const float time_duration,
	...
	const bool prefiltered, const bool debug)
{
  ...
  torch::Tensor out_means3D = torch::empty({P,3}, means3D.options());
  ...
  // === [수정] 반환 값을 받을 변수들 선언 ===
  int num_rendered_overlaps = 0;
  int num_gaussians_in_view = 0;

  if(P != 0)
  {
	  int M = sh.size(0) != 0 ? sh.size(1) : 0;
	  static CudaRasterizer::Rasterizer GsRasterizer;

      // === [수정] std::pair<int, int>로 반환 값을 받음 ===
	  std::pair<int, int> counts = GsRasterizer.forward(
	    geomFunc, binningFunc, imgFunc,
	    P, degree, degree_t, M,
		background.contiguous().data<float>(), W, H,
		means3D.contiguous().data<float>(), out_means3D.contiguous().data<float>(),
		sh.contiguous().data_ptr<float>(), colors.contiguous().data<float>(), 
		flows.contiguous().data<float>(), opacity.contiguous().data<float>(), 
		ts.contiguous().data_ptr<float>(), scales.contiguous().data_ptr<float>(),
		scales_t.contiguous().data_ptr<float>(), scale_modifier,
		rotations.contiguous().data_ptr<float>(), rotations_r.contiguous().data_ptr<float>(),
		...
		timestamp, time_duration, rot_4d, gaussian_dim, force_sh_3d,
		tan_fovx, tan_fovy, prefiltered,
		out_color.contiguous().data<float>(), out_flow.contiguous().data<float>(), 
		out_depth.contiguous().data<float>(), out_T.contiguous().data<float>(),
		radii.contiguous().data<int>(), debug
      );
      // === [수정] pair에서 값을 추출 ===
      num_rendered_overlaps = counts.first;
      num_gaussians_in_view = counts.second;
  }
  
  torch::Tensor covs3D_com = torch::zeros({P, 6}, float_opts);
  if(P != 0)
  {
    ...
    covs3D_com = torch::from_blob(geoState.cov3D, {P, 6}, float_opts);
  }
  
  // === [수정] 두 개의 int 값을 모두 튜플에 담아 반환 ===
  return std::make_tuple(num_rendered_overlaps, num_gaussians_in_view, out_color, out_flow, out_depth, out_T, radii, geomBuffer, binningBuffer, covs3D_com, out_means3D);
}


// === [수정] 반환 타입 튜플에 int를 하나 더 추가 ===
	...
	const bool prefiltered, const bool debug)
{
	const int P = means3D.size(0);
	const int D = degree;
	const int D_t = degree_t;
	const int M = sh.size(0) != 0 ? sh.size(1) : 0;
	torch::Tensor radii = torch::full({P}, 0, means3D.options().dtype(torch::kInt32));
	int* radii_ptr = radii.contiguous().data<int>();

	torch::Tensor out_means3D = torch::empty({P,3}, means3D.options());
	float* out_means3D_ptr = out_means3D.contiguous().data<float>();
	...
	// === [수정] std::tuple로 반환 값을 받음 ===
    auto [num_rendered_overlaps, num_gaussians_in_view, keys, values, depths] = GsRasterizer.preprocess_and_bin(
		geomFunc, P, D, D_t, M, image_width, image_height,
		means3D.contiguous().data<float>(),
		out_means3D_ptr,
		sh.contiguous().data_ptr<float>(), colors.contiguous().data<float>(), 
		opacity.contiguous().data<float>(), ts.contiguous().data_ptr<float>(), 
		scales.contiguous().data_ptr<float>(), scales_t.contiguous().data_ptr<float>(), 
		scale_modifier, rotations.contiguous().data_ptr<float>(), rotations_r.contiguous().data_ptr<float>(),
		cov3D_precomp.contiguous().data<float>(), viewmatrix.contiguous().data<float>(), 
		projmatrix.contiguous().data<float>(), campos.contiguous().data<float>(),
		timestamp, time_duration, rot_4d, gaussian_dim, force_sh_3d,
		tan_fovx, tan_fovy, prefiltered,
    ...
    // === [수정] 두 개의 int 값을 모두 튜플에 담아 반환 ===
	return std::make_tuple(num_rendered_overlaps, num_gaussians_in_view, keys, values, depths, geomBuffer, radii);
}
	...
	const bool debug)
{
	...
	auto float_opts = background.options().dtype(torch::kFloat32);
	...
	GsRasterizer.render_sorted(
		...
		flows.numel() > 0 ? flows.contiguous().data<float>() : nullptr,
	...
	return std::make_tuple(out_color, out_flow, out_depth, out_T);
}
